...
my %ids_hash;
#-----------------------[reads ids]--------------
   open ID,"genotype2_complete.list.txt" or die "Can't open ID file";
   while(<ID>) {
      my @lines = split(/\s+/);
      my $key=shift @lines;
...
open IN, "All_loci_seqs.fa" or die "Failed to open IN file\n";
#open OUT, ">>All_loci_seqs2.fa" or die "Failed to open OUT file\n";
...
while (<IN>) {
    ...
    next unless $head =~/(\d*[a-zA-Z]+\d+)/;
    # 在%ids_hash中查询记录
    if (exists $ids_hash{$1}){
		open OUT,">>$1.locis" || die "open OUT Failed!\n";
        print OUT ">$head\n$seq\n";
		 $hits++;
        }
